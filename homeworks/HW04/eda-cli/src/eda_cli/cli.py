...
def report(
    ...
    title: str = typer.Option("Отчёт по датасету", help="Заголовок отчёта"),
    min_missing_share: float = typer.Option(0.4, help="Порог доли пропусков, от 0 до 1")
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Повторение идентификаторов: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        f.write(f"- Наличие колонок, где все значения одинаковые: **{quality_flags['has_constant_columns']}**\n\n")
        ...
        if missing_df.empty:
            ...
            for col in summary.columns:
                if col.missing / summary.n_rows > min_missing_share:
                    f.write(f"- {col.name}: {round(col.missing / summary.n_rows, 2)}\n")
