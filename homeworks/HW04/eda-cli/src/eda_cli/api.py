...
class QualityFlagContent(BaseModel):
    too_few_rows: bool = Field(...)
    too_many_columns: bool = Field(...)
...
class QualityFlagsResponse(BaseModel):
    flags: QualityFlagContent = Field(...)
...
async def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    summary = summarize_dataset(df)
    miss_tables = missing_table(df)
    flags = compute_quality_flags(summary, miss_tables)
